...
7. PyPI credentials are short-lived OIDC credentials; no long-lived publishing token is stored.
8. The protected `pypi` environment requires human approval.
...
| R1-04 Supply chain | SPDX SBOM, provenance, Trusted Publishing, release assets | `forgeos-release.yml` | evidence artifact generated; OIDC publication gated | High: external configuration |
...
| R1-07 Tag and publish | Create version tag, approve environment, publish and verify | GitHub/PyPI | GitHub Release and PyPI files/digests/attestations match | High: irreversible public release |
...
read permissions in the build job. PyPI publication gets only `id-token: write`; GitHub Release gets
only `contents: write` after PyPI succeeds.
...
4. Create `testpypi` and `pypi` GitHub environments; require an approver for `pypi`.
5. Register PyPI Trusted Publisher using owner, repository, workflow `forgeos-release.yml`, and
   environment `pypi`; optionally register TestPyPI with environment `testpypi`.
6. Confirm ownership/availability of the package name `forgeos-harness` and configure a private
...
remote CI, GitHub attestations, or PyPI publication succeeded.
