<script lang="ts">
  ...
  type ProjectWorkspaceComponent = typeof import("$lib/components/ProjectWorkspace.svelte").default;
  ...
  type LazyWorkspaceKind = "arena" | "codeDiff" | "diagnostics" | "file" | "git" | "memory" | "project" | "settings" | "terminal";
  ...
  let ProjectWorkspaceView = $state<ProjectWorkspaceComponent | null>(null);
  ...
  const platformViewParamKey = "view";
  ...
  const ui = $derived.by(() => {
    ...
    return {
      ...
      engineeringWorkspace: locale === "zh-Hans" ? "持续开发工作区" : "Continuous development workspace",
      runtimeReady: locale === "zh-Hans" ? "运行节点就绪" : "Runtime ready",
      engineeringFlow: locale === "zh-Hans" ? "持续开发对话" : "Continuous development conversation",
      engineeringFlowHint: locale === "zh-Hans" ? "围绕当前项目持续协作，不强制拆分任务" : "Keep collaborating in the current project without mandatory task splits",
      flowObjective: locale === "zh-Hans" ? "项目上下文" : "Project context",
      flowObjectiveHint: locale === "zh-Hans" ? "持续保留目录、规则与目标" : "Keep the workspace, rules, and intent",
      flowExecution: locale === "zh-Hans" ? "实时执行" : "Live execution",
      flowExecutionHint: locale === "zh-Hans" ? "连续查看模型与工具输出" : "Follow model and tool output continuously",
      flowEvidence: locale === "zh-Hans" ? "代码变化" : "Code changes",
      flowEvidenceHint: locale === "zh-Hans" ? "按需查看文件、差异与命令" : "Inspect files, diffs, and commands on demand",
      flowAcceptance: locale === "zh-Hans" ? "工程工具" : "Engineering tools",
      flowAcceptanceHint: locale === "zh-Hans" ? "验证与评审按需调用" : "Use validation and review when needed",
      clearGoal: locale === "zh-Hans" ? "结束目标" : "Clear objective",
      pauseGoal: locale === "zh-Hans" ? "暂停执行" : "Pause",
      enterWorkspace: m.enter_workspace(),
      localGatewayNotice: m.local_gateway_notice(),
      password: m.password(),
      signIn: m.sign_in(),
      ...
      refresh: locale === "ko" ? "새로고침" : locale === "zh-Hans" ? "刷新" : "Refresh",
  ...
  const sessionSearchCopy = $derived.by(() => {
    ...
    if (locale === "zh-Hans") {
      return {
        placeholder: "搜索当前开发对话",
        hint: "搜索消息、推理、命令和工具调用。",
        noResults: "没有匹配内容。",
        loadMore: "加载更多结果",
        turn: "对话回合",
        results: "条结果",
  ...
  async function ensureSessionForComposer() {
    ...
    draftSelectedSkills = [];
    scheduleComposerTextareaResize();
    void rememberActiveProjectSession(created.id);
  ...
  const portalProjects = $derived.by(() => {
    ...
    for (const groupedSessions of discoveredByRoot.values()) {
      ...
      const rootPath = latest.cwd.trim().replace(/[\\/]+$/u, "");
      const baseName = rootPath.split(/[\\/]/u).filter(Boolean).at(-1) ?? "已发现项目";
      let name = baseName;
      let suffix = 2;
      ...
      discoveredProjects.push({
        ...
        rootPath,
        repoPath: latest.preferences?.gitRepoPath ?? null,
        lastSessionId: latest.id,
        lastOpenedAt: latest.updatedAt,
        settings: { model: latest.preferences?.model ?? null },
  ...
  const workspaceTabs = $derived.by(() => {
    const _locale = $localeSignal;
    const tabs: Array<{ id: WorkspaceTabId; label: string; kind: "chat" | "tasks" | "git" | "project" | "settings" | "computer" | "diagnostics" | "memory" | "git-diff" | "code-diff" | "file" | "terminal" }> = [
    ...
    if (projectTabOpen && activeSessionFolder) {
      tabs.push({
        id: "project",
        label: $activeLocale === "zh-Hans" ? "项目设置" : "Project",
        kind: "project"
      });
    }
    if (settingsTabOpen) {
  ...
  async function ensureLazyWorkspaceLoaded(kind: LazyWorkspaceKind) {
    ...
    if (kind === "project" && ProjectWorkspaceView) {
      return;
    }
    ...
    const pending = (async () => {
      ...
      if (kind === "project") {
        const module = await import("$lib/components/ProjectWorkspace.svelte");
        ProjectWorkspaceView = module.default;
        return;
      }
      if (kind === "settings") {
  ...
  function getProjectPortalRequestedFromUrl() {
    if (typeof window === "undefined") {
      return false;
    }
    return new URL(window.location.href).searchParams.get(platformViewParamKey) === "projects";
  }

  function syncPlatformViewInUrl(view: PlatformView) {
    if (typeof window === "undefined") {
      return;
    }
    const url = new URL(window.location.href);
    if (view === "portal") {
      url.searchParams.set(platformViewParamKey, "projects");
      url.searchParams.delete(sessionQueryParamKey);
      url.searchParams.delete(sessionNewParamKey);
    } else {
      url.searchParams.delete(platformViewParamKey);
    }
    const nextUrl = `${url.pathname}${url.search}${url.hash}`;
    const currentUrl = `${window.location.pathname}${window.location.search}${window.location.hash}`;
    if (nextUrl !== currentUrl) {
  ...
  async function bootstrap() {
    ...
    try {
      ...
      const projectPortalRequested = getProjectPortalRequestedFromUrl();
  ...
  async function selectSession(sessionId: string, profileId: string | null = null) {
    ...
    if (selectedSessionId === sessionId && conversation && (!profileId || currentSelectionProfileId === profileId)) {
      ...
      void rememberActiveProjectSession(sessionId);
  ...
  function goalPrimaryActionLabel(status: string | null | undefined) {
    return goalPrimaryAction(status) === "resume" ? ui.resume : ui.pauseGoal;
  }

  function goalStatusLabel(status: string | null | undefined) {
    if ($activeLocale !== "zh-Hans") {
      return status ?? "";
    }
    switch (status) {
      case "active":
  ...
  async function confirmDirectorySelection(nextPath: string) {
    if (browserPurpose === "session") {
      chooseDirectory(nextPath);
      return;
    }
  ...
  function openProjectSettings(folder: SessionFolder) {
    ...
    workspaceMenuOpen = false;
    void ensureLazyWorkspaceLoaded("project");
  }

  function closeProjectSettings() {
    projectTabOpen = false;
    if (activeWorkspaceTabId === "project") {
      activeWorkspaceTabId = "chat";
    }
  }
  ...
  function projectSectionForWorkspaceTab(): ProjectSection {
    if (activeWorkspaceTabId === "git" || activeWorkspaceTabId.startsWith("git-diff:") || activeWorkspaceTabId.startsWith("code-diff:") || activeWorkspaceTabId.startsWith("file:")) {
      return "code";
    }
    if (activeWorkspaceTabId === "project-validation") {
      return "validation";
    }
    if (activeWorkspaceTabId === "project-release") {
      return "release";
    }
  ...
  async function openProject(project: SessionFolder) {
    ...
    showArchivedSessions = false;
    syncSessionListStateInUrl();
    activeSessionFolder = project.name;
    activeDiscoveredProjectRoot = project.rootPath;
    syncSessionListStateInUrl();
  ...
  async function openProjectSection(project: SessionFolder, section: ProjectSection) {
    if (section === "settings" && !managedProjectByName(project.name)) {
      noticeText = `“${project.name}”由现有开发对话自动识别。请通过“新建项目”绑定该目录并纳入管理后，再修改项目设置。`;
      return;
    }
    if (activeSessionFolder !== project.name || platformView !== "project") {
  ...
  async function manageDiscoveredProject(project: SessionFolder) {
    if (readOnlyRole || !project.rootPath) {
  ...
  async function rememberActiveProjectSession(sessionId: string) {
    const project = managedProjectByName(activeSessionFolder);
    if (!project || project.lastSessionId === sessionId) {
      return;
    }
    try {
  ...
  async function openSessionFolder(folderName: string | null) {
    platformView = "project";
    syncPlatformViewInUrl("project");
  ...
  async function createSessionFolder() {
    ...
    await openBrowser(config?.defaults.cwd ?? null, "project");
  }
  ...
  function saveSessionFolderDefaults(folder: SessionFolder) {
    if (readOnlyRole || !config) {
      errorText = m.error_forbidden_role();
      return;
    }
  ...
  async function bindProjectRoot(folder: SessionFolder) {
    if (readOnlyRole || !config) {
      errorText = m.error_forbidden_role();
      return;
    }
  ...
  async function saveProjectSettings(project: SessionFolder, model: string | null) {
    if (readOnlyRole) {
  ...
  async function removeProject(project: SessionFolder) {
    ...
    noticeText = $activeLocale === "zh-Hans"
      ? "项目容器已移除，对话已保留在未归类列表。"
      : "Project removed; conversations were kept as unfiled.";
  }

  async function searchProjectConversations(project: SessionFolder, query: string) {
    const managed = Boolean(managedProjectByName(project.name));
    const response = await api.searchSessions(query, "full", false, null, 50, {
      pinnedOnly: false,
      runningOnly: false,
      queuedOnly: false,
      untaggedOnly: false,
      highlight: "all",
      tags: managed ? [project.name] : []
    });
    const results = "sessions" in response ? response.sessions : [];
  ...
  async function setSelectedSessionFolderMembership(folderName: string, inFolder: boolean) {
    ...
    const projectNames = new Set((config?.sessionOrganization.sessionFolders ?? []).map((folder) => folder.name));
    const nonProjectTags = selectedSessionSummary.tags.filter((tag) => !projectNames.has(tag));
    ...
    try {
      ...
      if (inFolder) {
        activeSessionFolder = folderName;
        void rememberActiveProjectSession(selectedSessionId);
      }
    } catch (error) {
      errorText = describeError(error);
    }
...
{#if authenticated !== true}
  <div class="forge-auth-shell relative flex h-[100dvh] min-h-[100dvh] w-full overflow-hidden text-gray-900">
...
{:else}
<div class="forge-shell flex h-[100dvh] min-h-[100dvh] w-full overflow-hidden font-sans text-gray-900" data-testid="workspace-shell">
  ...
  <aside
    ...
    class={[
      "forge-navigation h-full transition-all duration-300",
      isMobileLayout
        ? "fixed inset-y-0 left-0 z-[130] w-[min(22rem,calc(100vw-1.5rem))] max-w-[calc(100vw-1.5rem)] shadow-2xl"
        : "w-[19rem] min-w-[19rem] max-w-[21rem] flex-shrink-0"
    ]}
  >
    <SessionSidebar
      ...
      onSearchScopeChange={updateSessionSearchScope}
	      ...
	      onSelectSessionFolder={(folderName) => {
	        void openSessionFolder(folderName);
	      }}
	      ...
	      onSaveSessionFolderDefaults={(folder) => {
	        void saveSessionFolderDefaults(folder);
	      }}
  ...
  <main class="forge-workspace flex-1 flex flex-col h-full min-w-0 relative">
    ...
    <WorkspaceHeader
      ...
      tokenCountLabel={conversation?.tokenUsage ? `${formatTokenCount(conversation.tokenUsage.total.totalTokens)} ${$activeLocale === "zh-Hans" ? "令牌" : "tok"}` : null}
      readOnly={readOnlyRole}
      {ui}
                    ...
                    <div class="min-w-0 flex items-center gap-2">
                      ...
                      <div class="min-w-0">
                        ...
                        <p class="mt-0.5 text-[10px] text-gray-500">
                          {goalStatusLabel(conversation.goal.status)} · {formatTokenCount(conversation.goal.tokensUsed)}
                ...
                {#each visibleTranscriptTurns as turn, visibleTurnIndex (turn.id)}
                  ...
                  >
                    ...
                    <div class="flex gap-4">
                      <div class="forge-execution-node flex-shrink-0 w-9 h-9 rounded-xl text-white flex items-center justify-center shadow-sm mt-1" title={ui.flowExecution}><Shield size={18} /></div>
            ...
            <div class="pointer-events-auto max-w-3xl mx-auto w-full space-y-4">
              ...
              <div class="relative group">
                ...
                <form bind:this={composerPanelElement} class="composer-panel bg-white/95 border-2 border-gray-200 rounded-2xl shadow-2xl overflow-hidden transition-all duration-200 focus-within:-translate-y-0.5 focus-within:border-amber-400/70 focus-within:bg-white focus-within:shadow-[0_24px_60px_-34px_rgba(245,158,11,0.65)]" onsubmit={(event) => { event.preventDefault(); void submitComposer(); }}>
                  <textarea bind:this={composerTextareaElement} bind:value={draft} class="composer-textarea w-full min-h-[3rem] overflow-y-hidden border-none bg-transparent px-4 py-3 pr-12 text-sm leading-6 text-gray-800 placeholder-gray-400 outline-none transition-colors duration-150 focus:outline-none focus:ring-0 focus:placeholder:text-amber-500/70 resize-none sm:min-h-[3.25rem]" data-testid="composer-input" oninput={handleComposerInput} onkeydown={handleComposerKeydown} placeholder={composerQueueModeActive ? ui.queueFollowUpPlaceholder : ui.askCodex} readonly={readOnlyRole} rows="1"></textarea>
        ...
        <div class="h-full overflow-y-auto" style="background: var(--bg);">
          {#if ProjectWorkspaceView && activeProject}
            <ProjectWorkspaceView
              ...
              readOnly={readOnlyRole}
              onBindRoot={() => bindProjectRoot(activeProject)}
              onSave={(model) => saveProjectSettings(activeProject, model)}
              onRename={(nextName) => renameProject(activeProject, nextName)}
              onRemove={() => removeProject(activeProject)}
              onSearch={(query) => searchProjectConversations(activeProject, query)}
              onOpenSession={async (sessionId, profileId) => {
                activeWorkspaceTabId = "chat";
                await selectSession(sessionId, profileId);
              }}
            />
      ...
      {:else if (activeWorkspaceTabId === "project-release" || activeWorkspaceTabId === "project-operations") && activeProjectContext}
        ...
        {#if lifecycleProject}
          <ProjectLifecycleWorkspace
            ...
            surface={activeWorkspaceTabId === "project-release" ? "release" : "operations"}
            readOnly={readOnlyRole}
...
{#if browserOpen}
  <FolderBrowserDialog
    ...
    nameValue={browserPurpose === "project" ? projectNameDraft : ""}
    onBrowse={browseTo}
    onClose={() => {
      browserOpen = false;
      browserPurpose = "session";
