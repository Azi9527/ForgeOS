<script lang="ts">
  ...
  let {
    ...
    onSaveSessionFolderDefaults: (folder: SessionFolder) => void;
  ...
  let advancedOperatorSettingsOpen = $state(false);
  let notificationsOpen = $state(false);
  let searchPanelOpen = $state(false);
  let collapsedProjectGroups = $state<Record<string, boolean>>({});
  ...
  const ui = $derived.by(() => {
    ...
    return {
      ...
      serverCredentialsJson: isKorean ? "런타임 자격 증명" : "Runtime credential file",
      serverCredentialsJsonPlaceholder: isKorean ? "서버 자격 증명 파일 경로" : "Path to a local runtime credential file",
      serverCredentialsJsonDescription: isKorean
        ? "서버 파일시스템의 런타임 자격 증명을 현재 연결로 가져옵니다."
        : "Import runtime credentials from the server filesystem into the active connection.",
  ...
  const projectSessionGroups = $derived.by(() => {
    const groups = new Map<string, { key: string; label: string; path: string; sessions: SessionSummary[] }>();

    for (const session of sessions) {
      const path = session.cwd.trim().replace(/[\\/]+$/u, "");
      const key = path ? path.toLocaleLowerCase() : "__unassigned__";
      ...
      groups.set(key, {
        key,
        label: path.split(/[\\/]/u).filter(Boolean).at(-1) ?? ui.unfiledSessions,
        path,
        sessions: [session]
      });
    }
  ...
  function isProjectGroupCollapsed(group: { key: string; sessions: SessionSummary[] }) {
    const explicitState = collapsedProjectGroups[group.key];
    if (typeof explicitState === "boolean") {
      return explicitState;
    }
    return !group.sessions.some((session) => session.id === selectedId);
  }

  function toggleProjectGroup(group: { key: string; sessions: SessionSummary[] }) {
    collapsedProjectGroups[group.key] = !isProjectGroupCollapsed(group);
  }
  ...
  function sessionAccountBadgeLabel(session: SessionSummary) {
    ...
    if (profileLabel) {
      return profileLabel === "Local Codex" ? m.local_runtime() : profileLabel;
    }
    ...
    const fallbackLabel = profiles.find((profile) => profile.id === session.profileId)?.label ?? "";
    return fallbackLabel === "Local Codex" ? m.local_runtime() : fallbackLabel;
  }
  ...
  function operatorRoleLabel() {
    return webRole === "viewer" ? m.operator_viewer() : m.operator_admin();
  }

  function displayProfileLabel(label: string) {
    return label.trim() === "Local Codex" ? m.local_workspace() : label;
  }
...
<aside class="sidebar forge-sidebar flex h-full w-full min-w-0 flex-col transition-all">
  <div class="p-4 flex flex-col gap-4">
    ...
    <button
      ...
      type="button"
    >
      <div class="forge-create-task__icon flex h-7 w-7 items-center justify-center rounded-lg transition-colors">
        <Plus size={18} />
      </div>
      <span class="font-semibold">{ui.newThread}</span>
    </button>
  </div>
  ...
  <div class="px-4 pb-2 space-y-4">
	        ...
	        >
	          ...
	          >
	            ...
	            <div class={`sidebar-folder-item group/folder flex min-w-0 items-center gap-1 rounded-xl px-1 py-1 transition-colors ${
	              ...
	              <button
	                class="flex min-w-0 flex-1 items-start gap-2 rounded-lg px-1.5 py-1 text-left text-xs"
	              ...
	              >
	                ...
	                <span class="min-w-0 flex-1">
	                  ...
	                  {#if folder.rootPath}
	                    <span class="mt-0.5 block truncate text-[9px] font-normal opacity-60">{folder.rootPath}</span>
	                  {:else}
	                    <span class="mt-0.5 block truncate text-[9px] font-normal opacity-60">{getLocale() === "zh-Hans" ? "尚未绑定目录" : "No root bound"}</span>
	                  {/if}
	                </span>
	              ...
	              {#if activeSessionFolder === folder.name}
	                ...
	                <button
	                  aria-label={getLocale() === "zh-Hans" ? `打开 ${folder.name} 项目设置` : `Open settings for ${folder.name}`}
	                  class="sidebar-folder-action rounded-lg p-1 text-amber-600 transition-colors hover:bg-amber-100 disabled:cursor-not-allowed disabled:opacity-45"
	                  disabled={readOnly}
	                  onclick={() => onSaveSessionFolderDefaults(folder)}
	                  title={getLocale() === "zh-Hans" ? "项目设置、搜索与迁移" : "Project settings, search, and migration"}
	                  type="button"
	                >
	                  <Settings size={12} />
	                </button>
      ...
      {#if searchPanelOpen}
        <div
          ...
          class="sidebar-search-panel absolute left-0 right-0 top-[calc(100%+0.55rem)] z-[80] overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-[0_24px_64px_-24px_rgba(2,6,23,0.72)]"
        >
      ...
      {#each projectSessionGroups as projectGroup (projectGroup.key)}
        <section class="sidebar-project-group" data-project-key={projectGroup.key}>
          ...
          <button
            ...
            onclick={() => toggleProjectGroup(projectGroup)}
            title={projectGroup.path || ui.unfiledSessions}
            type="button"
          >
            ...
            {:else}
              <FolderOpen size={14} class="shrink-0" />
            {/if}
            <span class="min-w-0 flex-1 truncate text-[11px] font-bold">{projectGroup.label}</span>
            <span class="sidebar-project-count shrink-0 rounded-full px-1.5 py-0.5 text-[10px] font-semibold tabular-nums">
  ...
  <div class="forge-operator-dock p-3">
    <button
      aria-expanded={accountMenuOpen}
      bind:this={accountButtonElement}
      class="forge-operator-trigger group flex w-full items-center gap-3 rounded-xl p-2.5 text-left transition-all"
      onclick={() => {
    ...
    >
      <div class="forge-operator-avatar flex h-9 w-9 items-center justify-center rounded-xl transition-all">
        <ShieldCheck size={17} />
      </div>
      <div class="flex-1 min-w-0">
        <p class="truncate text-xs font-semibold leading-none text-white">{m.operator_center()}</p>
        <div class="mt-1.5 flex items-center gap-1.5">
          <span class="h-1.5 w-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.7)]"></span>
          <span class="truncate text-[9px] font-semibold uppercase tracking-[0.14em] text-white/40">{m.workspace_online()}</span>
        </div>
      </div>
      <span class="forge-operator-role rounded-full px-2 py-1 text-[8px] font-bold uppercase tracking-[0.14em]">{operatorRoleLabel()}</span>
      <div class="text-white/35 transition-colors group-hover:text-white/70">
        {#if accountMenuOpen}
          <ChevronDown size={16} />
        {:else}
          <ChevronUp size={16} />
    ...
    {#if accountMenuOpen}
      ...
      >
        <div class="sidebar-flyout-header p-4 border-b border-gray-100 flex items-center justify-between">
          <div>
            <h3 class="text-xs font-bold text-violet-500 uppercase tracking-widest mb-1">ForgeOS</h3>
            <p class="text-sm font-semibold text-gray-900">{m.operator_center()}</p>
        ...
        <div class="min-h-0 overflow-y-auto px-4 py-4 space-y-6 pr-3 scrollbar-thin">
          <div class="forge-control-summary rounded-2xl border p-4">
            <div class="flex items-start gap-3">
              <span class="forge-control-summary__icon inline-flex h-9 w-9 shrink-0 items-center justify-center rounded-xl">
                <ShieldCheck size={16} />
              </span>
              <div class="min-w-0">
                <p class="text-sm font-semibold text-gray-900">{m.workspace_control()}</p>
                <p class="mt-1 text-[11px] leading-5 text-gray-500">{m.workspace_control_description()}</p>
              </div>
            </div>
            <div class="mt-3 flex items-center gap-2 rounded-xl border border-emerald-100 bg-emerald-50 px-3 py-2 text-[10px] font-semibold text-emerald-700">
          ...
          <button
            aria-expanded={advancedOperatorSettingsOpen}
            class="forge-advanced-toggle flex w-full items-center justify-between gap-3 rounded-xl border px-3 py-2.5 text-left transition-all"
            onclick={() => (advancedOperatorSettingsOpen = !advancedOperatorSettingsOpen)}
            type="button"
          >
            <span class="flex items-center gap-2 text-xs font-semibold">
              <Settings size={14} />
              {advancedOperatorSettingsOpen ? m.hide_advanced_settings() : m.advanced_runtime_settings()}
            </span>
            <ChevronDown size={14} class={`transition-transform ${advancedOperatorSettingsOpen ? "rotate-180" : ""}`} />
          </button>

          {#if profiles.length > 0}
            <div class:hidden={!advancedOperatorSettingsOpen} class="space-y-3">
              <div class="flex items-center justify-between gap-2">
                <h4 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
                  <ShieldCheck size={10} /> {m.runtime_connections()}
                </h4>
                <button
                  ...
                  >
                    <button class="min-w-0 flex-1 text-left" onclick={() => onSelectProfile(profile.id)} type="button">
                      <div class="flex min-w-0 items-center gap-2">
                        <p class="truncate text-sm font-semibold">{displayProfileLabel(profile.label)}</p>
          ...
          <div class:hidden={!advancedOperatorSettingsOpen} class="space-y-4">
            <h4 class="text-[10px] font-bold text-gray-400 uppercase tracking-widest flex items-center gap-1.5">
        ...
        <div class="sidebar-flyout-footer p-2 border-t border-gray-100 bg-gray-50/50 space-y-1">
          <button 
            class:hidden={!advancedOperatorSettingsOpen}
...
<style>
  ...
  .sidebar-search-panel > div > .flex.bg-gray-100 {
    background: #0f172a !important;
    box-shadow: inset 0 0 0 1px rgba(71, 85, 105, 0.34);
  }

  .sidebar-search-scope__button,
  .sidebar-search-panel button {
    color: #94a3b8;
  }

  .sidebar-search-scope__button.bg-white,
  .sidebar-search-panel button.bg-white,
  .sidebar-search-panel button.bg-gray-900 {
    background: #334155 !important;
    color: #f8fafc !important;
  }

  .sidebar-search-filter,
  .sidebar-search-panel .bg-gray-50 {
    border-color: #334155 !important;
    background: #0f172a !important;
    color: #cbd5e1 !important;
  }

  .sidebar-search-filter:hover,
  .sidebar-search-panel button:hover {
    color: #f8fafc;
  }

  .sidebar-project-group + .sidebar-project-group {
    margin-top: 0.5rem;
  }
  ...
  :global(:root[data-theme="dark"]) .sidebar-search-panel {
    border-color: #334155 !important;
    background: #111827 !important;
    box-shadow: 0 28px 64px -24px rgba(2, 6, 23, 0.96) !important;
  }
