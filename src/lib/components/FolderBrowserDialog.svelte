...
<div aria-modal="true" class="fixed inset-0 flex items-center justify-center p-4 sm:p-6" role="dialog" style="z-index:var(--z-modal);">
  ...
  <div class="folder-dialog-card relative flex max-h-[80vh] w-full max-w-2xl flex-col overflow-hidden rounded-3xl bg-white shadow-2xl">
    <header class="folder-dialog-card__header flex items-center justify-between border-b border-gray-100 px-8 py-6">
      ...
      <button
        aria-label={closeLabel}
        class="rounded-xl p-2 text-gray-400 transition-all hover:bg-gray-100 hover:text-gray-900"
        onclick={onClose}
        type="button"
      >
        <X size={20} />
        ...
        <div class="space-y-4">
          {#if nameLabel}
            <label class="block space-y-2">
              <span class="text-[10px] font-bold uppercase tracking-widest text-gray-400">{nameLabel}</span>
              <input
                class="w-full rounded-xl border border-gray-200 bg-white px-4 py-2.5 text-sm font-semibold text-gray-800 outline-none transition-colors focus:border-amber-300 focus:ring-2 focus:ring-amber-100"
    ...
    <div class="folder-dialog-card__footer flex items-center justify-end border-t border-gray-100 bg-gray-50 px-8 py-4">
      <button
        class="rounded-xl bg-amber-600 px-6 py-2 text-xs font-bold text-white shadow-md transition-all active:scale-95 hover:bg-amber-700 disabled:cursor-not-allowed disabled:opacity-50"
        disabled={busy || !directoryPayload?.currentPath || (nameLabel !== null && !nameValue.trim())}
        onclick={() => directoryPayload?.currentPath && void onConfirm(directoryPayload.currentPath)}
        type="button"
      >
        {confirmLabel}
      </button>
