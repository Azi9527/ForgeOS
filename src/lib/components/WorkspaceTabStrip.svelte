<script lang="ts">
  import { Activity, Brain, FileDiff, FileText, FolderCog, GitBranch, History, Layout, MessageSquare, Monitor, Settings, Terminal, X } from "lucide-svelte";

  type WorkspaceTab = {
    id: string;
    label: string;
    kind: "chat" | "tasks" | "git" | "project" | "settings" | "computer" | "diagnostics" | "memory" | "git-diff" | "code-diff" | "file" | "terminal";
  };
...
<div class="workspace-tab-strip forge-tab-strip flex items-center gap-1 overflow-x-auto px-4 py-2 scrollbar-none">
  {#each tabs as tab (tab.id)}
    <button
      class={`ui-animated-button ui-animated-button--soft flex items-center gap-2 whitespace-nowrap rounded-lg px-3 py-1.5 text-xs font-semibold transition-all ${
        activeTabId === tab.id
          ? "forge-tab--active border text-gray-900 shadow-sm"
