...
export const api = {
  ...
  upsertSessionFolder(
    ...
    project: Partial<{
      ...
      markOpened: boolean;
      settings: AppConfigPayload["sessionOrganization"]["sessionFolders"][number]["settings"];
    }> = {}
  ) {
    return ws.request<{
      ...
      knownTags: string[];
      sessionFolders: AppConfigPayload["sessionOrganization"]["sessionFolders"];
    }>("sessionFolders/upsert", { name, pinned, ...project });
  },
  ...
  renameSessionFolder(name: string, nextName: string) {
    return ws.request<{
  ...
  async uploadProjectArtifact(projectName: string, version: string, sourceCommit: string | null, file: File) {
    ...
    return request<{ artifact: ProjectArtifact }>(apiPath("/project-artifacts"), {
      method: "POST",
      body: formData,
      credentials: "include"
    });
