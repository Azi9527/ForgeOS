...
pub(crate) async fn upsert_session_folder_payload(
    ...
    let root_path_patch = match params.get("rootPath") {
        ...
        Some(Value::String(path)) if !path.trim().is_empty() => {
            Some(Some(resolve_allowed_directory(state, path).await?))
        }
        Some(Value::String(_)) => Some(None),
        Some(_) => {
            return Err(api_error(
                StatusCode::BAD_REQUEST,
                "project rootPath must be a string or null",
            ));
        }
    };
    let repo_path_patch = match params.get("repoPath") {
        None => match root_path_patch.as_ref().and_then(Option::as_deref) {
            Some(root_path) => Some(resolve_git_repo_root(state, root_path).await.ok()),
            None => None,
        },
        Some(Value::Null) => Some(None),
        Some(Value::String(path)) if !path.trim().is_empty() => {
            Some(Some(resolve_git_repo_root(state, path).await?))
        }
        Some(Value::String(_)) => Some(None),
        Some(_) => {
            return Err(api_error(
                StatusCode::BAD_REQUEST,
                "project repoPath must be a string or null",
            ));
        }
    };
    let last_session_id_patch = match params.get("lastSessionId") {
        None => None,
        Some(Value::Null) => Some(None),
        Some(Value::String(session_id)) if !session_id.trim().is_empty() => {
            Some(Some(session_id.trim().to_string()))
        }
        Some(Value::String(_)) => Some(None),
        Some(_) => {
            return Err(api_error(
                StatusCode::BAD_REQUEST,
                "project lastSessionId must be a string or null",
            ));
        }
    };
    let settings_patch = match params.get("settings") {
        None => None,
        Some(Value::Object(settings)) => {
            let model = match settings.get("model") {
                ...
                Some(_) => {
                    return Err(api_error(
                        StatusCode::BAD_REQUEST,
                        "project settings.model must be a string or null",
                    ));
                }
            };
    ...
    let payload = with_ui_state_write(state, profile_id, |ui_state| {
        ...
        let created_at = current
            ...
            .unwrap_or(now);
        let root_path = root_path_patch.clone().unwrap_or_else(|| {
            current
                .get("rootPath")
                .and_then(Value::as_str)
                .map(str::to_string)
        });
        let repo_path = repo_path_patch.clone().unwrap_or_else(|| {
            current
                .get("repoPath")
                .and_then(Value::as_str)
                .map(str::to_string)
        });
        let last_session_id = last_session_id_patch.clone().unwrap_or_else(|| {
            current
                .get("lastSessionId")
                .and_then(Value::as_str)
                .map(str::to_string)
        });
        let last_opened_at = if mark_opened {
            Some(now)
        } else {
            current.get("lastOpenedAt").and_then(Value::as_u64)
        };
        let settings = settings_patch.clone().unwrap_or_else(|| {
            current.get("settings").cloned().unwrap_or_else(|| {
                json!({
                    "model": Value::Null
                })
            })
...
pub(crate) async fn delete_session_folder_payload(
    ...
    let payload = with_ui_state_write(state, profile_id, |ui_state| {
        ...
        if let Some(lifecycle_by_name) = ui_state
            .get_mut("projectLifecycleByName")
            .and_then(Value::as_object_mut)
        {
            lifecycle_by_name.remove(&name);
        }
...
pub(crate) async fn rename_session_folder_payload(
    state: &AppState,
    profile_id: &str,
    params: Value,
) -> ApiResult<Value> {
    let name = normalize_session_folder_name(params.get("name"))?;
    let next_name = normalize_session_folder_name(params.get("nextName"))?;
    if name == next_name {
        return Err(api_error(
            StatusCode::BAD_REQUEST,
            "The new project name must be different.",
        ));
    }

    let payload = with_ui_state_write(state, profile_id, |ui_state| {
        let existing_folders = session_folders_from_ui_state(ui_state);
        if !existing_folders
            .iter()
            .any(|folder| folder.get("name").and_then(Value::as_str) == Some(name.as_str()))
        {
            return Err(api_error(StatusCode::NOT_FOUND, "Project was not found."));
        }
        if existing_folders
            .iter()
            .any(|folder| folder.get("name").and_then(Value::as_str) == Some(next_name.as_str()))
        {
            return Err(api_error(
                StatusCode::CONFLICT,
        ...
        let folder = ui_state
            .get_mut("sessionFoldersByName")
            .and_then(Value::as_object_mut)
            .and_then(|folders| folders.remove(&name))
            .unwrap_or_else(|| {
                json!({
                    "name": name,
                    "pinned": false,
        ...
        let mut folder = folder.as_object().cloned().unwrap_or_default();
        folder.insert("name".to_string(), Value::String(next_name.clone()));
        folder.insert("updatedAt".to_string(), json!(now));
        ui_state
            .get_mut("sessionFoldersByName")
            .and_then(Value::as_object_mut)
            .ok_or_else(|| {
            ...
            .insert(next_name.clone(), Value::Object(folder));

        if let Some(lifecycle) = ui_state
            .get_mut("projectLifecycleByName")
            .and_then(Value::as_object_mut)
            .and_then(|entries| entries.remove(&name))
        {
            let mut lifecycle = lifecycle.as_object().cloned().unwrap_or_default();
            lifecycle.insert("projectName".to_string(), Value::String(next_name.clone()));
            ui_state
                .get_mut("projectLifecycleByName")
                .and_then(Value::as_object_mut)
                .expect("project lifecycle state is ensured")
                .insert(next_name.clone(), Value::Object(lifecycle));
        }
            ...
            .and_then(Value::as_object_mut)
        {
            for (session_id, meta) in entries {
                ...
                for tag in tags.iter_mut() {
                    if tag.as_str().is_some_and(|tag| tag.trim() == name) {
                ...
                if changed {
                    ...
                    tags.dedup();
                    migrated_session_ids.push(session_id.clone());
                }
            }
        }

        Ok(json!({
            "renamed": { "from": name, "to": next_name },
            "migratedSessionCount": migrated_session_ids.len(),
            "migratedSessionIds": migrated_session_ids,
            "knownTags": known_tags_from_ui_state(ui_state),
            "sessionFolders": session_folders_from_ui_state(ui_state)
        }))
    })
