...
async fn saves_session_folders_with_pin_state_and_empty_membership() {
    ...
    assert_eq!(
        folders[0].get("rootPath").and_then(Value::as_str),
        Some(workspace.canonicalize().unwrap().to_str().unwrap())
    );
    assert_eq!(
        folders[0].get("repoPath").and_then(Value::as_str),
        Some(workspace.canonicalize().unwrap().to_str().unwrap())
    );
    assert_eq!(
...
#[tokio::test]
async fn renames_project_and_migrates_conversation_membership() {
    let sandbox = unique_test_dir("rename-project");
    let workspace = sandbox.join("workspace");
    let codex_home = sandbox.join("codex-home");
    fs::create_dir_all(&workspace).unwrap();
    fs::create_dir_all(&codex_home).unwrap();
    ...
    for session_id in ["session-1", "session-2"] {
        ...
        .await
        .expect("project membership should be saved");
    }
    upsert_session_folder_payload(
        &state,
        "default",
        json!({
            "name": "alpha",
    ...
    let payload = rename_session_folder_payload(
        &state,
        "default",
        json!({ "name": "alpha", "nextName": "production" }),
    )
    .await
    .expect("project should be renamed");

    assert_eq!(
        payload.get("migratedSessionCount").and_then(Value::as_u64),
        Some(2)
    );
    let project = payload
        .get("sessionFolders")
        ...
        .and_then(|folders| {
            folders
                .iter()
                .find(|folder| folder.get("name") == Some(&json!("production")))
        })
        .expect("renamed project should be returned");
    assert_eq!(project.get("sessionCount").and_then(Value::as_u64), Some(2));
    assert_eq!(
        project.get("lastSessionId").and_then(Value::as_str),
        Some("session-2")
    );
