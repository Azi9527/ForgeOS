...
async function resolveBackendLaunch(config) {
  ...
  for (const candidate of explicitCandidates) {
    ...
    try {
      ...
      return { binary: resolved, cwd: packageRoot, release: null };
    } catch {
      // try the managed release and packaged binaries
    }
  }

  const installedRelease = await resolveInstalledGatewayRelease(stateDir, currentRustTargets());
  if (installedRelease) {
    return {
      binary: installedRelease.binaryPath,
      cwd: installedRelease.root,
...
async function startServerUnlocked(config) {
  ...
  const backendLaunch = await resolveBackendLaunch(config);
  ...
  const child = spawn(backendLaunch.binary, [], {
  ...
  try {
    ...
    await logHandle.close();
    throw new Error(`Failed to start gateway binary ${backendLaunch.binary}: ${error.message}`);
  }
  child.unref();
  ...
  await writeServerMeta({
    ...
    instanceToken,
    startedAt: new Date().toISOString(),
    command: backendLaunch.binary,
    cwd: backendLaunch.cwd,
    gatewayRelease: backendLaunch.release?.version ?? null,
    url: buildUrl(config)
  });
  await logHandle.close();
  try {
...
async function runGatewayCommand(config, argv) {
  const action = String(argv[0] ?? "status").trim();
  if (action === "--help" || action === "-h" || action === "help") {
    printGatewayUsage();
    return;
  }
  if (action === "status") {
    const [releaseState, server] = await Promise.all([
      readGatewayReleaseState(stateDir),
      readServerStatus(config)
    ]);
    console.log(`Gateway: ${server.verified ? "running" : server.running ? "unverified" : "stopped"}`);
    console.log(`Current release: ${releaseState.current?.version ?? "packaged"}`);
    console.log(`Previous release: ${releaseState.previous?.version ?? "none"}`);
    return;
  }
  if (action === "restart") {
  ...
  if (action === "install") {
    const bundleRoot = argv[1] ? path.resolve(argv[1]) : null;
    if (!bundleRoot) {
      throw new Error("An extracted gateway bundle directory is required.");
    }
    const previousState = await readGatewayReleaseState(stateDir);
    const nextState = await installGatewayBundle(stateDir, bundleRoot, currentRustTargets());
    const result = await restartAfterGatewaySwitch(config, previousState);
    console.log(`Installed gateway release ${nextState.current.version}.`);
    printUsage(config, result.pid, false);
    return;
  }
  if (action === "rollback") {
    const currentState = await readGatewayReleaseState(stateDir);
    const nextState = await rollbackGatewayRelease(stateDir);
    const result = await restartAfterGatewaySwitch(config, currentState);
    console.log(`Rolled back to gateway release ${nextState.current.version}.`);
    printUsage(config, result.pid, false);
    return;
  }
...
function parseCliInvocation(argv) {
  const knownCommands = new Set(["config", "status", "restart", "stop", "gateway", "tunnel"]);
  let command = "";
  let restArgs = [...argv];
  ...
  for (let index = 0; index < restArgs.length; index += 1) {
    ...
    if (command !== "tunnel" && command !== "gateway" && (argument === "--help" || argument === "-h")) {
  ...
  if (command !== "tunnel" && command !== "gateway" && remainingArgs.length > 0) {
...
async function main() {
  ...
  if (command === "gateway") {
    await runGatewayCommand(config, restArgs);
    return;
  }
